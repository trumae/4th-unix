...
| Added external editor support in Edit4tH()       | J.L. Bezemer     |301114|
| Added EditIntern(), EditExtern()                 | J.L. Bezemer     |011214|
...
+--------------------------------------------------+------------------+------+
  ...
  Copyright (C) 1994,2017 J.L. Bezemer
...
static short    Cmds     = 0;          /* issued commands */
static Hcode   *Object   = NULL;       /* compiled program */
static char     InterActive = FALSE;   /* true when using the menu */
static jmp_buf  Shell;                 /* buffer for shell error-return */
...
/*
This routine will call an external editor if EDIT4TH has been set.
It uses system() for that. If an error occurs, it returns TRUE.
*/

#ifndef ARCHAIC
  int EditExtern (char *editor, char *source)
#else
  int EditExtern (editor, source) char *editor; char *source;
#endif

{
  char *CmdLine;                       /* commandline buffer */
  int   RetVal;                        /* save return value */

  CmdLine = (char*)                    /* allocate enough memory */
            calloc (strlen (editor) + strlen (source) + 2, sizeof (char));

  if (CmdLine)                         /* if we got it, compose commandline */
     {
       CmdLine = strcat (strcat (strcpy (CmdLine, editor), " "), source);
       RetVal = system (CmdLine);      /* now call the external editor */
       free (CmdLine);                 /* free allocated memory */
       return (RetVal);                /* return the return value */
     }

  return (TRUE);                       /* signal we got an error */
}


/*
This routine calls the default internal 4tH block editor. It does this by
loading the included HX code and executing it.
*/

#ifndef ARCHAIC
  int EditIntern (int argn, char **args)
#else
  int EditIntern (argn, args) int argn; char **args;
#endif

{                                      /* signal we got an error */
  if ((Object = fetch_4th (EmbeddedHX)) == NULL) return (TRUE);

  fflush (stderr);                     /* prevent garbled output */
  (void) exec_4th (Object, argn, args, 0);
  fflush (stdout);                     /* execute the editor */

  Msg4tH ("Editing;");                 /* show any messages */
  DROPOBJECT;                          /* unload editor */
  return (FALSE);                      /* signal everything OK */
}
...
{
  char *ExtEdit;                       /* external editor command */
  char *SrcEdit = (argn > 0) ? args [0] : "new.scr";
  int   RetVal;                        /* compensate for missing arguments */

  if (IsSelected (CMD_LOAD))           /* check for command conflicts */
     ErrMsg (MBADCMD, "Exiting;");
  else
     {                                 /* environment variable set? */
       RetVal = ((ExtEdit = getenv ("EDIT4TH")) == NULL)
              ? EditIntern (argn, args) : EditExtern (ExtEdit, SrcEdit);
                                       /* select the proper editor */
       if (RetVal) ErrMsg (MCANNOT, "Exiting;", "edit", SrcEdit);
       else if (IsSelected (CMD_CGEN | CMD_SAVE | CMD_DUMP | CMD_EXEC))
                Enable (CMD_COMP);     /* if something went wrong */
     }                                 /* show message, otherwise */
}                                      /* check if compiler should be set */
...

{
  ...
  (void) dump_4th (Object, stdout, a, b);
  fflush (stdout);                     /* decompile object to stdout */
}                                      /* flush any output */
...
{
  unsigned short x;                    /* simple counter */
